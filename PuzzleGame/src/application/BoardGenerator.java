...
public class BoardGenerator {
    ...
    /*
     * @decs: check if this row is not able to store a car , this means it is a bad row
     */
    ...
    /*
     * @desc: a smarter random number generator
     *  use when we generate horizontal vehicles.
     *  make board generation faster
     *  use for find valid cell which is able to add a horizontal car or truck in this row
     *  @return: return -1 mean this is bad row that can not store this vehicle, otherwise good row
     */
    public int SmarterRandomNumberGenerater(final int rule, Vehicle[][] boardData, int whichRow, int size) {
    	...
    	i = 0;
    	while(i < 5) {
    		//for a car, if this cell and its next cell are both null, this cell is a valid cell
    		if(size == 2 && i < 5) {
    			if(boardData[i][whichRow] == null) {
    				if(boardData[i+1][whichRow] == null) {
    					set.add(i);
    			...
    			//for truck, if this cell and its i+1 and i+2 cell are all null, this cell is a valid cell
    			if(boardData[i][whichRow] == null) {
    				if(boardData[i+1][whichRow] == null && boardData[i+2][whichRow] == null) {
    					set.add(i);
    					//don't waste time to check i+1 && i+2 cell. for better generation, not waste space
    					i+=2;
    				}
    			}
    		}
    	...
    	/*
    	 * valid cell will be added to Set.
    	 * Then we randomly choose one of the valid cell to return
    	 */
    	while(!set.isEmpty()) {
    		n = random.nextInt(5);
    		if(!set.contains(n)) {
     ...
     * 		and randomly decide add a car or a truck, and its position
     * 		but if sometime the board is not able to add more horizontal vehicles, but "Hcounter" number
     * 		is not achieved yet. then don't care the remaining
     * some rule:
     * - for a car: its headX column must <= 4
     * - for a truck: its headX column must <= 3
     */
    public boolean addHorizontalVehicle(int Hcounter, Board b) {
    	...
    	rowSet.add(0); rowSet.add(1); rowSet.add(3); rowSet.add(4); rowSet.add(5); // keep row 2 empty
    	...
    	/*
    	 *  add horizontal vehicle
    	 *  -randomly decide which row, then which column
    	 *  -add it to board, if invalid, create again
    	 */
    	i = 0;
    	Vehicle Vtemp;
